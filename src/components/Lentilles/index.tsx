...
export default function Lentilles() {
    ...
    function handleMouseDown(){
        ...
        if(isMouseNear({x: originPos.current.x - focalLength.current, y: originPos.current.y})){
    ...
    function drawCanvas(canvas: HTMLCanvasElement){
        ...
        if((isMouseNear(Fpos) && !moving.current) || moving.current == focalLength){
            ctx.beginPath()
            ctx.arc(Fpos.x, Fpos.y, size/2, 0, 360)
            ctx.stroke()
        }

        drawLine(ctx, {x: Fpos.x, y: Fpos.y - size/5}, {x: Fpos.x, y: Fpos.y + size/5})
        ...
        if(rdelta.enabled){
            ...
            drawLine(ctx, {x: originX, y: Bpos.y}, {x: _Fpos.x * 10, y: _Fpos.x * 10 * mdelta + pdelta})
        ...
        <div id="lentilles-app">
            ...
            <div className="shadow-box" id="lentilles-inputs">
                <div id="lentilles-rayons">
                    {
                        rayons.map((rayon, i) => <div className="rayon-input" style={{userSelect: "none"}} key={i}>
                            <input 
                                type="color" 
                ...
                <div>
                    ...
                    <div>
                        ...
                        <input type="number" ref={focalLengthRef} onChange={(e) => {focalLength.current = Number(e.target.value); drawCanvas(canvasRef.current)}}/>
                    </div>
                </div>
            </div>
        </div>
