...
function Graph() {
    const [table, setTable] = useState<number[][]>([[1,2,3],[4,5,6]])
    ...
    return (<>
        ...
        <tbody>
        {
            table.map((col, i) => 
                <tr key={i}>
                    {col.map((row, j) => <td key={j}>
                        <input onKeyDown={(e) => InputKeyDown(e, i, j)} type="text" defaultValue={row}/>
                    </td>)}
                </tr>
            )
        }
        <tr>
            {
                new Array(Math.max(...table.map(c => c.length))).fill(0).map((_, i) => <td key={i} onClick={handlePlus}><span className="plus"><FontAwesomeIcon icon={faPlus}/></span><input type="text" /></td>)
            }
        </tr>
        </tbody>
        </table>
