...
function searchFunc(input) {
    let searchStr = stringtoSearch(input);
    if (searchStr && searchStr.length) {
        searchRes.innerHTML = ""
        let searchResults = []

        for(let el of elements){
            if(stringtoSearch(el.symbol) == searchStr || el.Z == searchStr){
                searchResults.push(el.Z)
            }
        }

        for(let el of elements){
            if(stringtoSearch(el.name).startsWith(searchStr) && !searchResults.includes(el.Z)){
                searchResults.push(el.Z)
            }
        }

        for(let el of elements){
            if(stringtoSearch(el.name).includes(searchStr) && !searchResults.includes(el.Z)){
                searchResults.push(el.Z)
            }
        }
